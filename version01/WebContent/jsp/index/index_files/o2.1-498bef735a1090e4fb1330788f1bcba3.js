...
},{"./assert":2}],5:[function(d,c){c.exports=function(f){function e(a){this.$el=f(a),this.$labels=this.$el.find("label"),this.$inputs=this.$el.find("input"),this.setInitialSelection(),this.$el.on("click","label",this.onSelectOption.bind(this))
}var g="segmented-control__option--selected";
return e.prototype.setInitialSelection=function(){var b=this.$inputs.filter(":checked");
b&&b.parent().addClass(g)
},e.prototype.onSelectOption=function(a){var h=f(a.currentTarget);
this.$labels.removeClass(g),h.addClass(g)
},e.bind=function(a){var i=this,h=[];
return f(a||"body").find(".segmented-control").each(function(){var b=new i(this);
h.push(b)
}),h
},e
}
},{}],6:[function(d,c){c.exports=function(f,e,h){function g(a,i){if(i&&(!/^#./.test(i)||!f(i).length)){throw new Error("The data-transition-at attribute value must be an id")
...
},{"./assert":2}],8:[function(i,h){function n(d,c){return"Selector "+d+" matches "+c.length+" elements"
}function m(){return ++j
...
switch(k(1===this.$element.length,n(a,this.$element)),k(1===this.$trigger.length,n(s,this.$trigger)),k(!this.$element.data()["o2-tooltip"],p),k(!this.$trigger.data()["o2-tooltip"],p),this.fixed||this.$element.appendTo(d("body")),q=this.$trigger.attr("title"),q&&(this.$trigger.attr("aria-label",q),this.$trigger.removeAttr("title")),this.$element.attr("aria-hidden","true").reflow(),r.event){case"none":break;
...
},f.prototype.show=function(a){a&&(a.preventDefault(),a.stopPropagation()),"true"===this.$element.attr("aria-hidden")&&(this.fixed||this.initializePosition(),this.$element.addClass("show").attr("aria-hidden","false").reflow(),this.emit("show",this),e&&!this.noEvents&&d("body").on("click."+this.uniqueId,{tooltip:this},l))
},f.prototype.hide=function(a){if(a&&a.preventDefault(),this.clearTimeout(),"false"===this.$element.attr("aria-hidden")){var g=function(){this.$element.removeAttr("aria-hidden").afterTransition(function(){this.$element.removeClass("show").attr("aria-hidden","true").reflow(),this.emit("hide",this,a)
}.bind(this)),d("body").off("click."+this.uniqueId)
}.bind(this);
...
},f.prototype.dispose=function(){this.hide(null),delete this.$element.data()["o2-tooltip"],this.$element.removeAttr("aria-hidden").detach(),delete this.$trigger.data()["o2-tooltip"],this.$trigger.off(".o2-tooltip")
},f.caretSize=12,f.prototype.initializePosition=function(){var q,p,v,u={},t=["top","bottom","left","right"],s=24,r=20;
for(p={height:parseInt(this.$trigger.attr("height")||this.$trigger.css("height"),10),width:parseInt(this.$trigger.attr("width")||this.$trigger.css("width"),10),offset:this.$trigger.offset()},v={height:parseInt(this.$element.css("height"),10),width:parseInt(this.$element.css("width"),10),classes:this.$element.attr("class").split(" ")},q=0;
q<v.classes.length&&(0!==v.classes[q].indexOf("tooltip-")||(v.classes=v.classes[q].split("-").slice(1),-1===t.indexOf(v.classes[0])));
q++){}switch(v.classes[0]){case"top":u.top=p.offset.top+p.height+f.caretSize;
break;
case"bottom":u.top=p.offset.top-v.height-f.caretSize;
break;
case"left":u.left=p.offset.left+p.width+f.caretSize;
break;
case"right":u.left=p.offset.left-v.width-f.caretSize;
break;
default:throw new ReferenceError("Unknown primary position class name referenced")
}if(u.hasOwnProperty("top")){switch(v.classes[1]){case"left":u.left=p.offset.left,p.width<s&&(u.left-=s-p.width/2);
break;
case"middle":u.left=p.offset.left+p.width/2-v.width/2;
break;
case"right":u.left=p.offset.left+p.width-v.width,p.width<s&&(u.left+=s-p.width/2);
break;
default:throw new ReferenceError("Unknown secondary position class name referenced")
}}else{switch(v.classes[1]){case"top":u.top=p.offset.top,p.height<r&&(u.top-=r-p.height/2);
break;
case"middle":u.top=p.offset.top+p.height/2-v.height/2;
break;
case"bottom":u.top=p.offset.top+p.height-v.height,p.height<r&&(u.top+=r-p.height/2);
break;
default:throw new ReferenceError("Unknown secondary position class name referenced")
}}this.$element.css(u)
},f
}
},{"./assert":2}],KYS9bz:[function(d,c){c.exports=function(v,u){function t(f){var e=[];
...
},{"./emitter":1,"./modules/matchMedia":3,"./modules/modal":4,"./modules/segmented-control":5,"./modules/subnav":6,"./modules/tabs":7,"./modules/tooltip":8,"./reflow":10,"./transitions":11}],10:[function(d,c){c.exports=function(b){return b.fn.reflow?b:(b.extend(b.fn,{reflow:function(){return this.each(function(){b(this).addClass("o2-reflow").removeClass("o2-reflow").height()
}),this
}}),b)
}
},{}],11:[function(d,c){c.exports=function(f,e){return e.Transitions?e:(e.Transitions={timeout:600,events:{transition:"transitionend",OTransition:"oTransitionEnd",WebkitTransition:"webkitTransitionEnd",MozTransition:"transitionend"},eventName:function(){var h,g=!1;
...
},{}],o2:[function(d,c){c.exports=d("KYS9bz")
},{}]},{},["KYS9bz"]);
